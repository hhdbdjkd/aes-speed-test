...
bool test_present_sp_I(const std::vector<uint8_t>& data, double av1, double sig1) {
    ...
    for (size_t i = 0; i < loops; i++) {
        ...
        for (size_t j = 0; j < 20; j++) {
            for (size_t k = 0; k < buffer.size(); k += BLOCK_SIZE) {
                present_encrypt_sp_I(&buffer[k], roundKeys);
            }
        }
        cy = __rdtsc() - cy;
    ...
    sig /= av;
    av = (av - t0) / (20 * len);
    std::cout << "present_SP:" << v << "   " << av << "   " << sig * 100 << "%" << std::endl;
    return 1;
}
...
void speed_present_sp_I(const std::vector<uint8_t>& data) {
    while (1) {
        ...
        if (test_present_sp_I(data, av, sig)) {
            break;
        }
    }
}
