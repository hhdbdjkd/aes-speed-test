...
bool test_align(char* data, double av1, double sig1) {
    ...
    rijndael_setup_align(inkey, keylen, 0, &key);
    ...
    for (size_t i = 0; i < loops; i++) {
        ...
        for (size_t j = 0; j < 20; j++) {
            for (size_t k = 0; k < 1024; k += BLOCK_SIZE) {
                rijndael_ecb_encrypt((const unsigned char*)&data[k], tmp, &key);
            }
        }
        cy = __rdtsc() - cy;
    ...
    sig /= av;
    av = (av - t0) / (20 * len);
    std::cout << "align:    " << v << "   " << av << "   " << sig * 100 << "%" << std::endl;
    return 1;
}
