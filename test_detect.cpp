...
bool test_detect(char* data, double av1, double sig1) {
    ...
    for (size_t i = 0; i < loops; i++) {
        ...
        for (size_t j = 0; j < 20; j++) {
            for (size_t k = 0; k < 1024; k += BLOCK_SIZE) {
                aes_detect((const unsigned char*)&data[k], tmp, &key);
            }
        }
        cy = __rdtsc() - cy;
    ...
    sig /= av;
    av = (av - t0) / (20 * len);
    std::cout << "detect:   " << v << "   " << av << "   " << sig * 100 << "%" << std::endl;
    return 1;
}
